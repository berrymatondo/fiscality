...
import type { PaiementQueueRow } from "@/components/dashboard/saisie/queue-row";
...
export function PaiementQueueTable({
  title,
  description,
  emptyMessage,
  rows,
                ...
                <tr key={`${row.domain}-${row.id}`} className="border-b border-border last:border-0">
                  ...
                  <td className="px-3 py-3">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      {mode === "dgf" && row.statut === "PUBLIE" && (
                        <>
                          <form action={row.renvoiValidationAction}>
                          ...
                          <form action={row.soumettreOpAction}>
                            <input type="hidden" name="id" value={row.id} />
                      ...
                      {mode === "bcc" && row.statut === "OP_SOUMIS" && (
                        <form action={row.confirmerExecutionAction}>
                          <input type="hidden" name="id" value={row.id} />
                          <button type="submit" className="text-xs font-medium text-emerald-700 hover:underline dark:text-emerald-400">
