...
import { saveSaisieMacro } from "../actions";

export default async function NouvelleSaisieMacroPage() {
  await requireRole(["CELLULE_MACRO"]);

  return (
    <UtilityShell eyebrow="Cellule macroéconomique" title="Nouvelle saisie" subtitle="Indicateurs macroéconomiques.">
      <Card>
          ...
          <form action={saveSaisieMacro} className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
              <Field label="PIB nominal (Mrd CDF)">
                <AmountInput name="pib" />
              </Field>
              <Field label="Croissance (%)">
                <AmountInput name="croissance" />
              </Field>
              <Field label="Inflation (%)">
                <AmountInput name="inflation" />
              </Field>
              <Field label="Taux de change (CDF/USD)">
                <AmountInput name="tauxChange" />
              </Field>
              <Field label="Cours du cuivre (USD)">
                <AmountInput name="coursCuivre" />
              </Field>
              <Field label="Cours du pétrole (USD)">
                <AmountInput name="coursPetrole" />
              </Field>
              <Field label="Réserves de change (mois d'importations)">
                <AmountInput name="reserves" />
              </Field>
            </div>
