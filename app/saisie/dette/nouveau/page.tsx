...
import { saveSaisieDette } from "../actions";

export default async function NouvelleSaisieDettePage() {
  await requireRole(["DETTE_DGDP"]);

  return (
    <UtilityShell eyebrow="Direction Générale de la Dette Publique" title="Nouvelle saisie" subtitle="Encours et service de la dette.">
      <Card>
          ...
          <form action={saveSaisieDette} className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
              <Field label="Encours dette extérieure (CDF)">
                <AmountInput name="encoursExterieure" />
              </Field>
              <Field label="Encours dette intérieure (CDF)">
                <AmountInput name="encoursInterieure" />
              </Field>
              <Field label="Ratio dette / PIB (%)">
                <AmountInput name="ratioDettePib" />
              </Field>
              <Field label="Service de la dette dû (CDF)">
                <AmountInput name="serviceDetteDu" />
              </Field>
              <Field label="Service de la dette payé (CDF)">
                <AmountInput name="serviceDettePaye" />
              </Field>
            </div>
