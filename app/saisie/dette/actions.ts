...
export async function saveSaisieDette(formData: FormData) {
  const session = await requireRole(["DETTE_DGDP"]);
  ...
  const existing = await prisma.saisieDette.findUnique({ where: { periode } });
  ...
  const data = {
    exercice,
    encoursExterieure: parseAmount(formData.get("encoursExterieure")),
    encoursInterieure: parseAmount(formData.get("encoursInterieure")),
    serviceDetteDu: parseAmount(formData.get("serviceDetteDu")),
    serviceDettePaye: parseAmount(formData.get("serviceDettePaye")),
    ratioDettePib: parseAmount(formData.get("ratioDettePib")),
    nouveauxEmprunts: String(formData.get("nouveauxEmprunts") ?? "").trim() || null,
  ...
  if (!existing) {
    const seq = (await prisma.saisieDette.count({ where: { periode } })) + 1;
    reference = formatReference("DGDP", periode, seq);
  }

  const saisie = await prisma.saisieDette.upsert({
    where: { periode },
  ...
  if (statut === "SOUMIS") {
    await notifySaisieSoumise("dette", saisie.id, saisie.periode);
  }
...
export async function deleteSaisieDette(formData: FormData) {
  const session = await requireRole(["DETTE_DGDP"]);
  const id = String(formData.get("id") ?? "");

  const saisie = await prisma.saisieDette.findUnique({ where: { id } });
  ...
  await prisma.saisieDette.delete({ where: { id } });
  await logAudit({
    domain: "dette",
    recordId: id,
    action: "Suppression",
...
export async function revertSaisieDetteToBrouillon(formData: FormData) {
  ...
  const saisie = await prisma.saisieDette.findUniqueOrThrow({ where: { id } });
  if (!canRevertToBrouillon(saisie.statut)) {
    throw new Error("Seule une saisie soumise peut être renvoyée en brouillon.");
  }

  await prisma.saisieDette.update({ where: { id }, data: { statut: "BROUILLON" } });
  await logAudit({
    domain: "dette",
    recordId: saisie.id,
  ...
  await notifySaisieStatutChange("dette", saisie.id, saisie.periode, saisie.auteurId, "Saisie renvoyée en brouillon");

  revalidatePath("/saisie");
}

export async function revertSaisieDetteToSoumis(formData: FormData) {
  const session = await requireSession();
  const id = String(formData.get("id") ?? "");
  ...
  await prisma.saisieDette.update({ where: { id }, data: { statut: "SOUMIS" } });
  await logAudit({
    domain: "dette",
    recordId: saisie.id,
  ...
  await notifySaisieStatutChange("dette", saisie.id, saisie.periode, saisie.auteurId, "Saisie renvoyée en soumission");

  revalidatePath("/saisie");
}

export async function transitionSaisieDette(formData: FormData) {
  const session = await requireSession();
  const id = String(formData.get("id") ?? "");
  ...
  if (isOwnerTransition(saisie.statut)) {
    if (role !== "DETTE_DGDP") throw new Error("Non autorisé.");
  } else if (!ADMIN_ROLES.includes(role)) {
    throw new Error("Non autorisé.");
  }

  await prisma.saisieDette.update({ where: { id }, data: { statut: next } });
  await logAudit({
    domain: "dette",
    recordId: saisie.id,
  ...
  if (next === "SOUMIS") {
    ...
    await notifySaisieStatutChange("dette", saisie.id, saisie.periode, saisie.auteurId, "Saisie validée");
  } else if (next === "PUBLIE") {
    await notifySaisieStatutChange("dette", saisie.id, saisie.periode, saisie.auteurId, "Saisie publiée");
  }

  revalidatePath("/saisie");
}

export async function getRapportSaisieDette(id: string): Promise<RapportData | null> {
  const session = await requireSession();
  const role = session.user.role as Role;
  ...
  const isAllowed = ADMIN_ROLES.includes(role) || role === "DETTE_DGDP";
